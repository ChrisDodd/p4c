...
class Visitor {
 public:
    ...
    void visit(const IR::Node *const &n, const char *name, int cidx) {
        ...
        auto t = apply_visitor(n, name);
        if (t != n) visitor_const_error(); }
    void visit(IR::REF<IR::Node> &n, const char *name = 0) { n = apply_visitor(n, name); }
    void visit(const IR::REF<IR::Node> &n, const char *name = 0) {
        auto t = apply_visitor(n, name);
        if (t != n) visitor_const_error(); }
    void visit(IR::REF<IR::Node> &n, const char *name, int cidx) {
        ctxt->child_index = cidx;
        n = apply_visitor(n, name); }
    void visit(const IR::REF<IR::Node> &n, const char *name, int cidx) {
        ctxt->child_index = cidx;
        auto t = apply_visitor(n, name);
        if (t != n) visitor_const_error(); }
...
#define DECLARE_VISIT_FUNCTIONS(CLASS, BASE)                            \
    ...
    void visit(IR::REF<IR::CLASS> &n, const char *name = 0);            \
    void visit(const IR::REF<IR::CLASS> &n, const char *name = 0);      \
    void visit(IR::REF<IR::CLASS> &n, const char *name, int cidx);      \
    void visit(const IR::REF<IR::CLASS> &n, const char *name, int cidx);\
