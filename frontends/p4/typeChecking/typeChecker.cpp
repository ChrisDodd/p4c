...
const IR::Expression *TypeInference::constantFold(const IR::Expression *expression) {
    ...
    // FIXME -- fix DoConstantFolding to take a DeclarationLookup * instead of a refMap, 
    // or not need a refmap at all.
    DoConstantFolding cf(nullptr, typeMap, false);
    auto result = expression->apply(cf, getContext());
...
std::pair<const IR::Type *, const IR::Vector<IR::Argument> *> TypeInference::checkExternConstructor(
    ...
    auto rettype = new IR::Type_Var(IR::ID(unique.newName("R"), "<returned type>"_cs));
    auto callType =
